...
public class TargetGUIGroup : MonoBehaviour
{
    ...
    void SetTargets()
    {
        ...
        SetDescription(LevelManager.THIS.levelData.target.Description);
        ...
        if (sprites != null)
            for (var i = 0; i < sprites.Length; i++)
            {
                ...
                if(LevelManager.THIS.levelData.subTargetsContainers.Any(x => x.extraObject != null && x.extraObject.name == sprites[i].name))
                    LevelManager.THIS.levelData.subTargetsContainers.First(x => x.extraObject.name == sprites[i].name).color = i;
