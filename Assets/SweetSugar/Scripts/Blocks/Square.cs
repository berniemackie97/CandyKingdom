...
/// </summary>
public class Square : MonoBehaviour, ISpriteRenderer, IField
{
...
//        if (!nextSq?.IsNone() ?? false)
    ...
    public void CheckFallOut()
    {
        ...
        {
            var nxtSq = Vector2.Distance(this.transform.position,Item.transform.position)>0.3f ? this: GetNextSquareRecursively();//GetNextSquareRecursively();
            if (nxtSq )
            {
                if (nxtSq.CanGoInto())
                {
    ...
    /// </summary>
    /// <returns></returns>
    public Square GetNextSquareRecursively()
    {
        ...
        if (sq != null)
        {
            if (sq.IsNone())
                return this;
            if (sq.Item != null || !sq.CanGoInto())
                return this;
            sq = sq.GetNextSquareRecursively();
        }
        else
            sq = this;
        return sq;
    ...
    {
        return !GetSubSquare()?.canMoveIn ?? false;
    }
    /// <summary>
    ...
    {
        return (!GetSubSquare().CanGoInto() || !GetSubSquare().CanGoOut() || GetSubSquare().undestroyable);
    }
    ...
    {
        return (GetSubSquare()?.canMoveOut ?? false) && (!GetSubSquare()?.undestroyable ?? false) ;
    }
    /// <summary>
    ...
    {
        return (GetSubSquare()?.canMoveIn ?? false) && (!GetSubSquare()?.undestroyable ?? false) ; //TODO: none square falling through
    }
    /// <summary>
    /// Next move event
    /// </summary>
    ...
    public bool IsHaveSolidAbove()
    {
        ...
        return seq.Any(square => square != this && (square.GetSubSquare().CanGoOut() == false || square.GetSubSquare().CanGoInto() == false ||
                                                    square.GetSubSquare().undestroyable || square.IsNone()));
    }
    /// <summary>
